...
<div class="container container-body">
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            @else
                                <span>-</span>
                            @endif
                            <button class="btn btn-secondary" type="submit" name="generate_access_token" value="1" onclick="return confirm('Generate a new read token? The previous read token will stop working.');">
                        ...
                        <td>
                            @if(($pluginStatus['exists'] ?? false) === true)
                                <select class="form-control field-select" name="plugin_state">
                                    ...
                                    <option value="not_installed">Uninstall</option>
                                </select>
                            @else
    ...
    @if($activeTab === 'guide')
        ...
        <div class="sectionBody">
            <p><strong>Template Registry</strong> - это API и генератор реестра для Evolution CMS. Он связывает шаблоны, контроллеры, view-файлы и TV, и дает HTTP API для чтения и записи.</p>

            <h3 style="margin-top:1.5rem;">1. API access</h3>
            <p>API по умолчанию требует manager-сессию. Включите или выключите его на вкладке <strong>Access</strong>.</p>
            <p>Для локальных инструментов и AI-агентов можно сгенерировать read token на вкладке <strong>Access</strong> и передавать его в заголовке <code>X-Template-Registry-Token</code>. Токен показывается только один раз после генерации.</p>
            ...
            <p>Write API выключен по умолчанию. Включите его на вкладке <strong>Access</strong> через <em>Write API status</em>.</p>
            ...
            <p>На вкладке <strong>Registry preview</strong> отображается статистика: количество шаблонов, TV, ресурсов, ClientSettings-полей и статус установленных расширений.</p>

            <h3 style="margin-top:1.5rem;">6. Auto-generate plugin</h3>
            <p>Плагин автоматически перегенерирует реестр при сохранении TV или шаблонов в админке. Установите его на вкладке <strong>Access</strong> и включите.</p>
